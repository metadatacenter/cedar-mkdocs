...
The two screenshots that follow present slightly different views of the CEDAR workspace. 
...
* (E) The large white middle pane lists the resources you can view. Controls (A), (B), (C), (H), and (I) affect what content you see in this pane. You can sort the items in this pane by clicking on the header of each column, or by clicking on the sort icon (M). 
* (F) Each resource in the middle pane has its own menu dropdown, selected by these vertical triple-dots at the right of the resource. With this menu you can rename, copy, move, share, and perform many other functions with the item.
...
* (J) The number of items listed in the middle pane is shown here. If the number of items is larger, only a subset are shown. Scroll  the display up to see more items.
* (K) In this location is an icon to let you switch the middle pane to cards or list format. The cards format is shown in the cards view below. 
* (L) The 'i' icon will bring up an information panel to the right side of your display, providing additional information (metadata) about the selected resource. If no resource is selected, the 'i' icon presents metadata about the folder (B) shown in the middle pane. The last image below shows the information panel for one of these artifacts.
* (M) The sort icon lets you choose the sort criteria for the middle pane contents.
...
The cards-based view presents each resource in the middle pane as a card rather than a row in a table. Every card shows the resource's type icon, its title, and any tags it carries, along with the three-dot menu (F) for renaming, copying, moving, or sharing the item. This format gives a more visual, at-a-glance layout of a folder's contents; you can switch between the cards and list formats at any time using the layout toggle (K).
