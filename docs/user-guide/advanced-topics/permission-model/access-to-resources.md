...
1. Check whether the user has a direct grant on the resource. If so, include the role from that
   grant.
2. Identify every group to which the user belongs. For each group, check whether the group has a
   direct grant on the resource. Include the role from every grant found.
3. Examine every folder that contains the resource, from its immediate parent to the top of the
   folder tree. Include every role granted on those folders to the user or to one of the user's
   groups.
4. Check whether the user owns any folder that contains the resource. If so, treat the user as having
